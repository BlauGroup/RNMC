...
LatticeState::LatticeState(const LatticeState &lattice_in) : homogeneous(lattice_in.homogeneous),
                                                             lattice(lattice_in.lattice ? (new Lattice(lattice_in.lattice->latconst,
                                                                                                       lattice_in.lattice->xhi / lattice_in.lattice->latconst,
                                                                                                       lattice_in.lattice->yhi / lattice_in.lattice->latconst,
                                                                                                       lattice_in.lattice->zhi / lattice_in.lattice->latconst))
                                                                                        : (new Lattice(0)))
{
} // copy constructor
                                                  ...
                                                  int react_id, std::unique_ptr<Lattice> &lattice, int site_id)
{
    ...
    {
        p = factor_two * num_one * num_two * k;
    }
    assert(p > 0);
...
void LatticeReactionNetwork::clear_site(std::unique_ptr<Lattice> &lattice,
                                        std::unordered_map<std::string,
                                                           std::vector<std::pair<double, int>>> &props,
                                                   ...
                                                   int reaction_index)
{
    ...
    for (int m = 0;
         ...
         m++)
    {
        if (reaction.phase_products[m] == Phase::SOLUTION)
            state[reactions[reaction_index].products[m]]++;
    }
} // update_state_solution()
                                                  ...
                                                  std::unique_ptr<Lattice> &lattice)
{
    ...
    else
    {
        ...
        {
            p = factor_duplicate * factor_two * state[reaction.reactants[0]] * (state[reaction.reactants[0]] - 1) * k;
        }
        else
        {
            p = factor_two * state[reaction.reactants[0]] * state[reaction.reactants[1]] * k;
        }
    }
    assert(p >= 0);
    return p;
...
} // get_butler_volmer_rate_coefficient()
