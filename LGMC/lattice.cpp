...
void Lattice::structured_lattice()
{
    ...
    {
        ilo = (xlo / latconst);
        while ((ilo + 1) * latconst > xlo)
            ilo--;
        ilo++;
        ihi = (xhi / latconst);
        while (ihi * latconst <= xhi)
            ihi++;
        ihi--;
    }
    ...
    {
        jlo = (ylo / latconst);
        while ((jlo + 1) * latconst > ylo)
            jlo--;
        jlo++;
        jhi = (yhi / latconst);
        while (jhi * latconst <= yhi)
            jhi++;
        jhi--;
    }
    ...
    {
        klo = (zlo / latconst);
        while ((klo + 1) * latconst > zlo)
            klo--;
        klo++;
        khi = (zhi / latconst);
        while (khi * latconst <= zhi)
            khi++;
        khi--;
    }
...
void Lattice::structured_connectivity()
{
    ...
    for (int i = 0; i < nsites; i++)
    {
        ...
        for (int neigh = 0; neigh < maxneigh; neigh++)
        {
            ...
            if (!is_xperiodic && (xneigh < xlo || xneigh > xhi))
                continue;
            if (!is_yperiodic && (yneigh < ylo || yneigh > yhi))
                continue;
            if (!is_zperiodic && (zneigh < zlo || zneigh > zhi))
                continue;
